...
##        #plt.plot(self.fft_mean);
        #plt.plot(10*np.log10(np.square(self.fft_dec[-2, 1:])/50));
        #plt.plot(self.faxis, 10*np.log10(np.square(self.fft_all)/50), linewidth = 0.5)
        plt.plot(10*np.log10(np.square(self.fft_all[int(self.nfft*self.overlap/self.dec_factor):2*int(self.nfft*self.overlap/self.dec_factor)])/50))
##        #plt.xlim(self.start_freq/1e6,self.stop_freq/1e6)
    ...
    def writeNew(self, filename):
        self.fileData = filename + '.sigmf-data'
        self.f = open(self.fileData, "wb")
        self.fileMeta = filename + '.sigmf-meta'
    ...
    def write(self):
        data1 = 10*np.log10(np.square(self.fft_all)/50)
        data1.astype(float)
        data1.tofile(self.f)

        self.flo.append(self.faxis[0])
        self.fhi.append(self.faxis[-1])
        self.datetimekey.append(dt.datetime.utcnow().isoformat()+'Z')
        self.length.append(len(data1))

    def writeMeta(self, location):
        self.f.close()
...
sweepRun = sweep(970e6, 1000e6, 'max')
sweepRun.capture()
